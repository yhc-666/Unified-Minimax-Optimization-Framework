"""
Using Neural Collaborative Filtering (NCF) to generate the complete rating matrix.
"""
import pickle
import numpy as np
import sys
...
# NCF model - proper rating prediction (no sigmoid)
model = NCF(user_num=user_num, item_num=item_num, embedding_size=embedding_size, l2_reg_lambda=l2_reg_lambda)
...
import os
os.makedirs("semi-synthetic/data_ncf", exist_ok=True)

file = open("semi-synthetic/data_ncf/predicted_matrix", "wb")
pickle.dump(prediction, file)
pickle.dump(user_num, file)
pickle.dump(item_num, file)
file.close()
print(f"\nPredicted matrix saved to data_ncf/! Shape: {prediction.shape}")
